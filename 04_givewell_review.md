...
\begin{align}
  ...
  \end{array}
        \right.\\
  \widetilde{F}^{r}_{16} &= 0.8F_{19} + 0.2\frac{F_{20}}{F_{21}}F^{r}_{22}
        \label{eq:18}
\tag{18}
\end{align}
...
- $F^{r}_{23}$:  DtW intermediate intensity adjustment for specific region (`F3, 1, G14`)  
- $F^{r}_{24}$:  Proportion of S. Haem in region (`F3, 1, J14`)  
...
- $F^{r}_{25}$: Calculated [location] intensity adjustment (`F3, 4, H27`).  
