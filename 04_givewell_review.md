...
- $F^{r}_{16}$: DtW weight for worm intensity by location, adjusted (`F3, 1, X14`)  
...
- $F_{19}$: Average intermediate intensity adjustment across regions and charities (`F3, 1, G50`) - $F^{r}_{21}$: Average Goblal Burden of Decease adjustment across regions and charities (`F3, 1, L50`)     
...
- $F^{r}_{29}$: Prevalence rates of each infection by location (`F3, 4, BE:27`).  
