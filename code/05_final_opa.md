...
<img src="/Users/ellashi/Documents/GitHub/opa-deworming/code/main_pe.png" alt="Main Policy Estimate" width="100%" />
...
This report is part of an open policy analysis project on deworming interventions. Open policy analysis (OPA) is a framework that aims to make policy analyses transparent and reproducible [@hoces2020framework]. The goal is to clearly show how the analysis was conducted and what is the best representation of the facts for policy makers to base their discussion on. In addition to this primary goal, OPA aims at facilitating the re-use of analyses across similar settings, and at shedding light on how the evidence generated by research is used specifically in a policy analysis. Following OPA principles, this project contain three components:
  ...
  2. A detailed report that describe how to obtain the policy estimate, and

  3. [A repository](https://github.org/bitss/opa-deworming)  that contains all the materials to reproduce the analysis with minimal effort.  

This report corresponds to the second component of this OPA project. The goal of this report is to provide a complete description of the analysis behind the results presented to inform a policy discussion on deworming interventions. This description aims to inform policy analysts and researchers on all the methodological choices and how to reproduce the analysis in its entirety. In order to document all the steps while not overwhelming the reader, the report is displayed in a layered fashion. The first layer consists of a narrative description of the analysis. The second layer contains equations that show how each piece of the analysis was carried out. The third and final layer displays the code used to operationalize each equation. All this information is contained in this document using the format of dynamic documentation, so interested readers can access the source file of the report and reproduce the entire document in their own computing environment.  
...
In this second approach, benefits follow the same principle as in approach 1 (increase in lifetime earnings), but we use updated data on the effects on the labor market outcomes. Instead of projecting a trend of earnings into the future (after the estimated impact of the 10 year follow-up), this analysis uses additional data from 15 and 20 year follow-ups after the original intervention.  Costs are fairly similar to approach 1, with the addition that in the second approach, the costs also account for several rounds of treatment required for effective deworming.  Additionally. the interest rate is updated to current values of return on (Kenyan) government bonds and inflation.
...
<tr>
   ...
   <td style="text-align:right;"> NA </td>
   <td style="text-align:right;"> 517.6 </td>
  </tr>
</tbody>
</table>
