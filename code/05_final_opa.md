...
This report is part of an open policy analysis project on deworming interventions. Open policy analysis (OPA) is a framework that aims to make policy analyses transparent and reproducible [@hoces2020framework]. The goal is to clearly show how the analysis was conducted and what is the best representation of the facts for policy makers to based their disussion on. In addition to this primary goal, OPA aims at facilitating the re-use of analyses across similar settings, and at sheding light on how is that the evidence generated by research is used specifically in a policy analysis. Following OPA principles, this project contain three components: 
  ...
  2. A detailes report that describe how to obtain the policy estimate, and  
  3. [A repository](https://github.org/bitss/opa-deworming) that contains all the materials to reproduce the analyses with minimal effort.   
  
This report corresponds to the second component of this OPA project. The goal of this report is to provide a complete description of the analysis behind the results presented to inform a policy discussion. This description aims to inform policy analysts and researchers on the all the methodological choices and how to reproduce the analysis in its entirity. In order to document all the steps while not overwealmimg the reader, the report is displayed in a layered fashion. The first layer consist of a narrative description of the analysis. The second layer contains equations that show how each piece of the analysis was carried out. The third and final layer displays the code used to operationalize each equation. All this information is contained in this document using the format of dynamic documentation, so interested reader can access the source file of the report and reproduce the entire document in their own computing enviroment.  
