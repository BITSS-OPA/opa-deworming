...
To estimate the costs, we follow a similar approach to @givewell. The default cost is the per unit of treatment across all countries. This is obtained as the weighted average of per unit costs ($c_{i}$) in all countries were Evidence Action currently has data on implementation of deworming interventions. 
...
```r
# - inputs: nothing
# - outputs: function that computes the weighted sum of country costs
...
<br>
AQUI VOY: Flesh out points bellow, then review code (below), generate some intermediary result (table of costs and/or locations)

Weights are based on number of treated individuals. 

Per capita costs as computed as total costs divided by number of treated children. 

Total costs are computed as the country level costs over the folling payers: 
