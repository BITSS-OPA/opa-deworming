...
The second approach does not report benefits and costs separately. With all these elements the main result from the original analysis that is comparable with the results discussed here is the NPV of `r df_research_so$param[9]` (table `r df_research_so$tablenum[9]`, column `r df_research_so$table_col[9]`, and row `r df_research_so$table_row[9]`)[^8]. This result corresponds to a social internal rate of return of `r paste0(df_research_so$param[8], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[8]` - and in the appendix at table `r df_research_so$tablenum[8]`, column `r df_research_so$table_col[8]`, and row `r df_research_so$table_row[8]`). Following the steps described in this section, this analysis obtains the same result (`r a2_all_pe` and `r paste0(irr_in * 100, "%")`, respectively, without rounding).
...
-->
In the KLPS, on average, one person gives birth to **`r tot_chld_so` children in a lifetime (XXX)**. The deworming treatment reduced the under five mortality rates of children of dewormed students by **`r gamma_mort_so*1000` (per 1,000 children)**. Thus, the treatment to one person roughly averted the death of `r tot_chld_so * gamma_mort_so` children among the `r tot_chld_so` children born from one person in the treatment group. From a perspective of cost, the direct deworming costs per one student are `r round(s2_new_ppp_in*q2_in, 2)` (2017 USD PPP), given the treatment period of `r years_of_treat_0_so` years and take-up rate of `r q2_in*100`% [^10]. So, we could simply say that `r tot_chld_so * gamma_mort_so` lives can be protected by spending $`r round(s2_new_ppp_in*q2_in, 2)` per treatment to one person. The below sections further explain the costs and benefits of saved children of dewormed students with additional parameters.

[^10]: The direct deworming costs per one student is different from \$`r format(round(s2_new_in * q2_in,2), nsmall = 2)` shown in approach 2 because approach 2 and 4 use different conversion rates[^4].
...
```{r chldsurv_benefits, echo=print_code}
...
Under approach 4, and using the same assumptions as above, the present value of child survival benefits is $`r chldsurv_hlth_in`.
