...
date: "<center><div class='mysubtitle'>`r format(Sys.time(), '%d %B, %Y')` <br><img height = '60px' src = './images/BITSS_logo_horizontal.png'><img height='60px' src='./images/CEGA_logo.png'><a href = 'http://www.bitss.org/opa/projects/deworming/'><img height = '60px' src = './images/OPA_layers.png'></a></div></center>"
...
3.  [A repository](`r links['dw_repo', 'url']`) that contains all the materials needed to reproduce, or update the interactive app, the open policy report, and the overall analysis.
...
> *Note: This is Version 1.0 of the OPA. Please kindly report any errors in the dynamic document [here](`r links['issues', 'url']`).*
...
The actual value varies across approaches depending on the time and country chosen. For example, Approach 1 uses the return from government bonds and the inflation rate in Kenya for 2016, while approaches 2, 3, and 4 use the same country's value for 2019. This results in discount rates of `r paste(round(100 * interest_in,2), "%", sep="")` for approaches 1, `r paste(round(100 * interest_new_in,2), "%", sep="")` for approaches 2, 3, and 4.
...
```{r curency-conversion, echo=print_code}
...
The estimated externality effect ($\lambda_{2}$) reflects the additional hours worked due to individuals who did not receive the treatment but still saw reductions in the likelihood of infection due to lower worm prevalence in their community. This parameter is not estimated by gender, so the report repeats its value two times. All the components of the equation \\ref{eq:`r equationIndex + 1`} come from @baird2016worms. The externality effects are adjusted by the coverage and saturation from the original study.
...
The coverage ($R$) is defined as the fraction, among all neighboring schools (within a 6 km radius) treated within the study. Since the treatment was applied to approximately two-thirds of the population, $R = `r round(coverage_so, 2)`$[^4].
...
For this setting, @kremer2007illusion (Page 48, Table 1, Panel C, Col 1, Row 3) estimated that take-up with full subsidy ($Q(full)$) was `r q_full_so`. @miguel2004worms (Table 3 and footnote 18) observed minimal to no take-up without subsidy ($Q(0)$); hence it is assigned the value of `r q_zero_so`.
...
Without externalities, @baird2016worms obtained a present value of benefits of `r df_research_so$param[1]` (table `r df_research_so$tablenum[1]`, column `r df_research_so$table_col[1]`, and row `r df_research_so$table_row[1]`). Including externalities, they obtain a present value of benefits of `r df_research_so$param[3]` (table `r df_research_so$tablenum[3]`, column `r df_research_so$table_col[3]`, and row `r df_research_so$table_row[3]`). Following the steps described in this section, this analysis obtains the same result (`r pv_benef_no_ext_in` and `r pv_benef_yes_ext_in` respectively without rounding; `r currency_f(pv_benef_no_ext_in, year_var=2016)` and `r currency_f(pv_benef_yes_ext_in, year_var=2016)` in 2017 USD PPP).
...
With complete subsidy, the relevant costs represent the total direct costs of deworming in USD. The take-up with full subsidy ($Q_2$) comes from @kremer2007illusion and takes the value of `r q_full_so`.
...
With complete subsidy, the costs of the intervention become the total direct costs of deworming each child (in USD). The original study [@baird2016worms] identifies the unit cost to be \$`r unit_cost_so` per year. Adjusting for purchasing power and inflation, the report gets a per capita cost of \$`r round(currency_f(price_var = unit_cost_so, year_var = 2018),2)`. Adding all indirect costs over an average `r round(years_of_treat_0_so,1)` years of treatment, the average cost of deworming each child over the entire treatment period is \$`r format(round(s2_new_in,2), nsmall = 2)`, and after accounting for a take-up rate of `r format(round(q2_in,2), nsmall = 2)` results in an average cost of \$`r format(round(s2_new_in * q2_in,2), nsmall = 2)` (2017 USD PPP).

#### Indirect costs: additional years of education and its costs for government

The indirect cost on the education system is calculated similarly to approach 1: the cost per student is multiplied by the increase in school attendance due to deworming. The cost of additional schooling is computed as the the annual cost of schooling per children times the number of additional years of schooling due to deworming. This analysis assumes that pressure is added to educational institutions for a maximum of nine years, starting at year zero, for a total of 10 years. The cost per student ($K$) is updated with new information on annual teacher salary (including benefits)[^6], \$`r format(round(currency_f(price_var=teach_sal_new_so, year_var=2017),0), scientific=FALSE, big.mark = ",")` (also adjusted for PPP), and the same average number of students per teacher (`r n_students_so`).
...
Hence, the cost of schooling each child for an additional year is now \$`r round(currency_f(price_var=teach_sal_new_so, year_var=2017)/n_students_so, 1)` (2017 USD PPP).
...
get_irr <- function(rate_var) {
  ...
  pv_costs_f(
     ...
     s2_var = s2_new_f(
       ...
       unit_cost_local_var = currency_f(price_var = unit_cost_so, year_var = 2018),
       ex_rate_var = 1,
       year_of_treat_var = years_of_treat_t_so
     ),
     q2_var = q_full_so
...
The second approach does not report benefits and costs separately. With all these elements the main result from the original analysis that is comparable with the results discussed here is the NPV of `r df_research_so$param[9]` (2017 USD PPP; table `r df_research_so$tablenum[9]`, column `r df_research_so$table_col[9]`, and row `r df_research_so$table_row[9]`)[^7]. This result corresponds to a social internal rate of return of `r paste0(df_research_so$param[8], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[8]` - and in the appendix at table `r df_research_so$tablenum[8]`, column `r df_research_so$table_col[8]`, and row `r df_research_so$table_row[8]`). Following the steps described in this section, this analysis obtains the same result (`r a2_all_pe` and `r paste0(irr_in * 100, "%")`, respectively, without rounding).
...
In the KLPS, on average, one person gives birth to **`r round(tot_chld_so, 2)` children in a lifetime (XXX)**. The deworming treatment reduced the under-five mortality rates of children of dewormed students by **`r gamma_mort_so*1000` (per 1,000 children)**. Thus, the treatment to one person roughly averted the death of `r round(tot_chld_so * gamma_mort_so, 2)` children among the `r round(tot_chld_so,2)` children born from one person in the treatment group. From a perspective of cost, the direct deworming costs per one student are `r round(s2_new_in*q2_in, 2)` (2017 USD PPP), given the treatment period of `r years_of_treat_0_so` years and take-up rate of `r q2_in*100`%. So, we could simply say that `r tot_chld_so * gamma_mort_so` lives can be protected by spending $`r round(s2_new_in*q2_in, 2)` per treatment to one person. The below sections further explain the costs and benefits of saved children of dewormed students with additional parameters.
...
Adjusted by the inflation and PPP to USD in 2017, the $M_p$ for revealed preference is `r round(currency_f(price_var = cp_daly_rp_so, year_var = 2011), 2)`, and the $M_p$ for stated preference is `r round(currency_f(price_var = cp_daly_sp_so, year_var = 2016), 2)`, respectively (2017 USD PPP). The reader can modify the values for $M_p$ in the interactive app of this OPA. 
...
For example: When treating parents at early ages with deworming reduced their child’s mortality rate by `r gamma_mort_so*100`% ($γ_{t}$), if `r round(fert_yr_25_so[1],3)` children are born per dewormed individual 0 years after the deworming ($F_{t = 0}$), and one child in Kenya averts `r round(yll_pc_in, 2)` YLLs during age 0-64 ($H$), we can estimate the discounted monetary value for one dewormed student at the time of $t = 0$. We sum up the benefits using the fertility rate in a specific year $t$ ($F_{t}$: for $t = 0, 1, ...,25$ ). Then, we multiply them by the cost per DALY averted ($M_p$) (revealed preference: \$`r round(currency_f(price_var = cp_daly_rp_so, year_var = 2011), 2)`; stated preference: \$`r round(currency_f(price_var = cp_daly_sp_so, year_var = 2016), 2)`):

```{=tex}
\begin{equation}
IGMB_{t} = \left( \$`r round(currency_f(price_var = cp_daly_rp_so, year_var = 2011), 2)`;
\$`r round(currency_f(price_var = cp_daly_sp_so, year_var = 2016), 2)` \right) 
\left( \underbrace{ `r gamma_mort_so` \times `r round(fert_yr_25_so[1],3)` \times `r round(yll_pc_in, 2)`}_{t = 0} + \dots \\
\dots + \underbrace{`r gamma_mort_so` \times `r round(fert_yr_25_so[11],3)` \times `r round(yll_pc_in, 2)`}_{t = 10} + \dots \\
\dots + \underbrace{`r gamma_mort_so` \times `r round(fert_yr_25_so[25],3)` \times `r round(yll_pc_in, 2)`}_{t = 25} \right)\\
\end{equation}
```


```{r intgen, echo=print_code}
...
Under approach 4, and using the same assumptions as above, the present value of child survival benefits is \$`r round(app4_pv_benef_intgen_rp_in,2)` for revealed preference and \$`r round(app4_pv_benef_intgen_sp_in,2)` for stated preference. 


### Costs

Approach 4 adopts the direct deworming costs under Approach 2 (\$`r format(round(s2_new_in * q2_in,2), nsmall = 2)` (2017 USD PPP), `r round(years_of_treat_0_so,1)` years of treatment, and take-up rate of `r format(round(q2_in,2), nsmall = 2)`) and the deworming costs including Evidence Action's technical assistance under Approach 3 (\$`r round(currency_f(price_var = costs1_p2_in, year_var = 2018), 2)` (2017 USD PPP)). 


```{r intgen_deworming-costs, purl=FALSE, echo=FALSE}
library(rootSolve)

pv_costs_a4_in <- pv_costs_f(
   periods_var = periods_so,
   delta_ed_var = 0,
   interest_r_var = interest_new_in,
...
a4_intgen_rp_pe  <- NPV_pe_f(benefits_var = app4_pv_benef_intgen_rp_in, 
                          costs_var = pv_costs_a4_in)

a4_intgen_sp_pe  <- NPV_pe_f(benefits_var = app4_pv_benef_intgen_sp_in, 
                          costs_var = pv_costs_a4_in)

irr_intgen_rp_in <- (multiroot(function(x) 
  (pv_benef_f(
    earnings_var = 0,
    intgen_var = intgen_app4_rp_in,
    interest_r_var = x,
  ...
  pv_costs_f(
     ...
     s2_var = s2_new_f(
       interest_var = x,
       unit_cost_local_var = currency_f(
         price_var = unit_cost_so, year_var = 2018),
       ex_rate_var = 1,
       year_of_treat_var = years_of_treat_t_so),
     q2_var = q_full_so)
  ), .1, maxiter=1000000, positive = T))$root

irr_intgen_sp_in <- (multiroot(function(x) 
  (pv_benef_f(
    earnings_var = 0,
    intgen_var = intgen_app4_sp_in,
    interest_r_var = x,
...
The original analysis in **XXX(2022)** implies the NPV of `r df_research_so$param[12]` (table `r df_research_so$tablenum[12]`, column `r df_research_so$table_col[12]`, and row `r df_research_so$table_row[12]`) for revealed preference and the NPV of `r df_research_so$param[13]` (table `r df_research_so$tablenum[13]`, column `r df_research_so$table_col[13]`, and row `r df_research_so$table_row[13]`) for stated preference. These results corresponds to social internal rates of return of `r paste0(df_research_so$param[10], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[10]` - and in the appendix at table `r df_research_so$tablenum[10]`, column `r df_research_so$table_col[10]`, and row `r df_research_so$table_row[10]`) and $1.9\times10^{12}%$ (located as an inline result in the paper - also in `r df_research_so$figure_num[11]` - and in the appendix at table `r df_research_so$tablenum[11]`, column `r df_research_so$table_col[11]`, and row `r df_research_so$table_row[11]`) for revealed preference and stated preference, respectively. Following the steps described in this section, this approach 4 obtains the same results (`r a4_intgen_rp_pe` and `r paste0(round(irr_intgen_rp_in * 100,2), "%")` for revealed preference and `r a4_intgen_sp_pe` and `r paste0(round(irr_intgen_sp_in * 100,2), "%")` for stated preference, respectively). 
...
|----------|----------------------------------------|-----------------------------------------|---------------------------|---------------------------|---------------------------|---------------------------|
| 1.1      | @baird2016worms with no externalities  | Treatment, Education                    | `r round(a1_all_pe, 1)`   | `r round(a1_tax_pe, 1)`   |`r round(a1_all_pe_2017usdppp, 1)`   | `r round(a1_tax_pe_2017usdppp, 1)`   |
| 1.2      | @baird2016worms with externalities     | Treatment, Education with externalities | `r round(a1_x_all_pe, 1)` | `r round(a1_x_tax_pe, 1)` | `r round(a1_x_all_pe_2017usdppp, 1)` | `r round(a1_x_tax_pe_2017usdppp, 1)` |
| 2.1      | @klps4 with no externalities           | Treatment, Education                    | `r round(klps4_2_pe, 1)`  | `r round(klps4_1_pe, 1)`  |`r round(klps4_2_pe, 1)`  | `r round(klps4_1_pe, 1)`  |
| 3.1      | 1.1 + prevalence + length of treatment | Treatment (EA)                          | `r round(ea1_pe, 1)`      | \-                        |`r round(ea1_pe_2017usdppp, 1)`      | \-                        |
| 3.2      | 1.2 + prevalence + length              | Treatment (EA)                          | `r round(ea2_pe, 1)`      | \-                        | `r round(ea2_pe_2017usdppp, 1)`      | \-                        |
| **3.3**  | **2.1 + prevalence + length**          | **Treatment (EA)**                      | **`r round(ea3_pe, 1)`**  | **-**                     | **`r round(ea3_pe_2017usdppp, 1)`**  | **-**                     |
...
npv_all_sim <-   sim_data1_f(
            ...
            unit_cost_local_new_var2 = currency_f(price_var = unit_cost_so, year_var = 2018),
            unit_cost_local_new_sd_var2 = currency_f(price_var = unit_cost_so, year_var = 2018) * 0.1  ,  
            ...
            new_costs_var2          = round(cost1_in,2),       #costs2_ea_in, NO CHANGE
            new_costs_sd_var2       = round(cost1_in,2) * 0.1, #costs2_ea_in * 0.1,
            countries_var2          = list("india", "kenya", "nigeria", "vietnam")

            # yll_var2                = yll_so ,           # app4
            # yll_sd_var2             = yll_so * 0.1 ,
            # pop_var2                = pop_so ,
            # pop_sd_var2             = pop_so * 0.1 ,
            # cp_daly_rp_var2         = cp_daly_rp_so,
            # cp_daly_rp_sd_var2      = cp_daly_rp_so * 0.1 ,
            # cp_daly_sp_var2         = cp_daly_sp_so ,
            # cp_daly_sp_sd_var2      = cp_daly_sp_so * 0.1 ,
            # gamma_mort_var2         = gamma_mort_so ,
            # gamma_mort_sd_var2      = gamma_mort_sd_so ,
            # fert_yr_25_var2         = fert_yr_25_so  ,
            # fert_yr_25_sd_var2      = fert_yr_25_so * 0.1 ,
            # life_exp_var2           = life_exp_so  ,
            # life_exp_sd_var2        = life_exp_so * 0.1 ,
                        )
