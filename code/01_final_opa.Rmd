...
```{=tex}
\begin{equation}
IGMB = \left( \$`r round(currency_f(price_var = cp_daly_rp_so, year_var = 2011), 2)`;
\$`r round(currency_f(price_var = cp_daly_sp_so, year_var = 2016), 2)` \right) 
\left\{ \left( \frac{1}{1 + 0.05}\right)^{t = 0} \times `r gamma_mort_so` \times `r round(fert_yr_25_so[1],3)` \times `r round(yll_pc_in, 2)` + \dots \\
\dots + \left(  \frac{1}{1 + 0.05}\right)^{t = 10} \times `r gamma_mort_so` \times `r round(fert_yr_25_so[11],3)` \times `r round(yll_pc_in, 2)` + \dots \\
\dots + \left(  \frac{1}{1 + 0.05}\right)^{t = 25} \times `r gamma_mort_so` \times `r round(fert_yr_25_so[25],3)` \times `r round(yll_pc_in, 2)` \right\}\\
\end{equation}
```
...
Under approach 4, and using the same assumptions as above, the present value of child survival benefits is \$`r round(intgen_app4_rp_in,2)` for revealed preference and \$`r round(intgen_app4_sp_in,2)` for stated preference. 
...
pv_benef_intgen_in <- intgen_app4_f(interest_new_in,
                    periods_chldb_25_so,
                    gamma_mort_so,
                    fert_yr_25_so,
                    yll_pc_in,
...
a4_intgen_pe  <- NPV_pe_f(benefits_var = pv_benef_intgen_in, 
                          costs_var = pv_costs4_1_in)

get_irr <- function(rate_var) {
  intgen_app4_f(interest_new_in,
                    periods_chldb_25_so,
                    gamma_mort_so,
                    fert_yr_25_so,
                    yll_pc_in,
