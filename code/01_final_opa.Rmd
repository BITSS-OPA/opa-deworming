...
ave_death_u5_so, gbd_YLD_u5$val, gbd_YLD_u5$pop, gbd_YLD_5t65$val, gbd_YLD_5t65$pop, gbd_YLL_5t65$val, gbd_YLL_5t65$pop

h_in <- (5 - ave_death_u5_so) * (1 -  sum(gbd_YLD_u5$val/5 ) / sum(gbd_YLD_u5$pop) )  +
...
\left( \underbrace{ `r gamma_mort_so` \times `r round(fert_yr_25_in[1],3)` \times `r round(yll_pc_in, 2)`}_{t = 0} + \dots \\
\dots + \underbrace{`r gamma_mort_so` \times `r round(fert_yr_25_in[11],3)` \times `r round(yll_pc_in, 2)`}_{t = 10} + \dots \\
\dots + \underbrace{`r gamma_mort_so` \times `r round(fert_yr_25_in[25],3)` \times `r round(yll_pc_in, 2)`}_{t = 25} \right)\\
\end{equation}
```
