...
This report provides a complete description of the analysis behind the results presented to inform a policy discussion on deworming interventions. It describes how to reproduce the analysis in its entirety, and includes all the methodological choices involved. In order to document all the steps without overwhelming the reader, the report is displayed in a layered fashion. The first layer consists of a narrative description of the analysis. The second layer, which appears after clicking in ![screenshot](images/show_details.png?display%20=%20inline-block) contains equations that show how each piece of the analysis was carried out. The third and final layer displays the code used to operationalize each equation. This information is contained within this document using dynamic documentation [@xie2015dynamic], so interested readers can access the report's source file and easily reproduce the entire document in their own computing environments.
...
Hence, the cost of schooling each child for an additional year is now \$`r round(currency_f(price_var=teach_sal_new_so, year_var=2017)/n_students_so, 1)` (USD).
...
The second approach does not report benefits and costs separately. With all these elements the main result from the original analysis that is comparable with the results discussed here is the NPV of `r df_research_so$param[9]` (table `r df_research_so$tablenum[9]`, column `r df_research_so$table_col[9]`, and row `r df_research_so$table_row[9]`)[^7]. This result corresponds to a social internal rate of return of `r paste0(df_research_so$param[8], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[8]` - and in the appendix at table `r df_research_so$tablenum[8]`, column `r df_research_so$table_col[8]`, and row `r df_research_so$table_row[8]`). Following the steps described in this section, this analysis obtains the same result (`r a2_all_pe` and `r paste0(irr_in * 100, "%")`, respectively, without rounding).
...
Under approach 3, and using the same assumptions as above, the net present value of benefits is: `r round(app3_pv_benef_no_ext_in, 2)` and `r round(app3_pv_benef_yes_ext_in, 2)` when using benefits of approach 1 without and with externalities. The net present value of benefits is `r round(app3_pv_benef_all_new_in, 2)` when using the benefit structure of approach 2.
...
Adjusted by the inflation and PPP to USD in 2017, the $M_p$ for stated preference is `r round(currency_f(price_var = cp_daly_s_so, year_var = 2016), 1)` and the $M_p$ for revealed preference is `r round(currency_f(price_var = cp_daly_r_so, year_var = 2011), 1)` , respectively (2017 USD PPP). The reader can modify the values for $M_p$ in the interactive app of this OPA. 
...
For example: When treating parents at early ages with deworming reduced their child’s mortality rate by `r gamma_mort_so*100`% ($γ_{t}$), if `r fert_yr_25_so[1]` children are born per dewormed individual 0 years after the deworming ($F_{t = 0}$), and one child in Kenya averts `r round(yll_pc_in, 2)` YLLs during age 0-64 ($H$), we can estimate the discounted monetary value for one dewormed student at the time of $t = 0$. We sum up the benefits using the fertility rate in a specific year $t$ ($F_{t}$: for $t = 0, 1, ...,25$ ). Then, we multiply them by the cost per DALY averted ($M_p$) (stated preference: \$`r round(currency_f(price_var = cp_daly_s_so, year_var = 2016), 2)`; revealed preference: \$`r round(currency_f(price_var = cp_daly_r_so, year_var = 2011), 2)`):
...
```{r intgen_mort_benefits, echo=print_code}
...
Under approach 4, and using the same assumptions as above, the present value of child survival benefits is \$`r round(intgen_mort_s_in,2)` for stated preference and \$`r round(intgen_mort_r_in,2)` for revealed preference. 
...
| 1.2      | @baird2016worms with externalities     | Treatment, Education with externalities | `r round(a1_x_all_pe, 1)` | `r round(a1_x_tax_pe, 1)` |
| 2.1      | @klps4 with no externalities           | Treatment, Education                    | `r round(klps4_2_pe, 1)`  | `r round(klps4_1_pe, 1)`  |
| 3.1      | 1.1 + prevalence + length of treatment | Treatment (EA)                          | `r round(ea1_pe, 1)`      | \-                        |
