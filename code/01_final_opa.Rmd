...
# url
# TODO: Fix issue with path down below
# knitr::include_graphics("code/images/main_pe.png")
```
...
The monetary value of health benefits ($M_p$) is the saved cost when one DALY is averted. @klps5 uses two approaches: revealed preference and stated preference. Adjusted by the inflation and PPP to USD in 2017, the $M_p$ for revealed preference is \$`r round(currency_f(cost_per_daly_so, t_var = 2011), 2)`, and the $M_p$ for stated preference is \$`r round(currency_f((12 * WTP_child_med_so) / ex_rate_2016_so, t_var = 2016), 2)`, respectively (2017 USD PPP). The reader can modify the values for $M_p$ in the interactive app of this OPA.

The treatment effects on under-five mortality for the children of the dewormed cohort ($γ_{t}$) is `r gamma_mort_so` (@klps5. Table 1, Panel A, Column 1, Row 1). The fertility rate $t$ years after deworming ($F_{t}$) is calculated as the number of children born per individual at $t$ years after the deworming intervention. (@klps5. Figure A.4).
...
Under approach 4, and using the same assumptions as above, the present value of child survival benefits is \$`r round(app4_pv_benef_rp_in,2)` for revealed preference and \$`r round(app4_pv_benef_sp_in,2)` for stated preference.

### Costs

The costs for approach four follow the costs of @klps5 (which adopts the direct deworming costs under @baird2016worms, \$`r format(round(s2_new_in * q2_in,2), nsmall = 2)` (2017 USD PPP), `r round(years_of_treat_0_so,1)` years of treatment, and take-up rate of `r format(round(q2_in,2), nsmall = 2)`).

```{r comp-repro-app4, purl=FALSE, echo=FALSE}
library(rootSolve)
...
a4_intgen_rp_pe  <- NPV_pe_f(benefits_var = app4_pv_benef_rp_in, 
                          costs_var = pv_costs_a4_in)

a4_intgen_sp_pe  <- NPV_pe_f(benefits_var = app4_pv_benef_sp_in, 
                          costs_var = pv_costs_a4_in)

irr_intgen_rp_in <- (multiroot(
  function(x)
    (
      pv_benef_f(
        earnings_var = 0,
...
The original analysis in [@klps5] estimtates an NPV of `r df_research_so$param[12]` (table `r df_research_so$tablenum[12]`, column `r df_research_so$table_col[12]`, and row `r df_research_so$table_row[12]`) for revealed preference and an NPV of `r df_research_so$param[13]` (table `r df_research_so$tablenum[13]`, column `r df_research_so$table_col[13]`, and row `r df_research_so$table_row[13]`) for stated preference. These results corresponds to social internal rates of return of `r paste0(df_research_so$param[10], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[10]` - and in the appendix at table `r df_research_so$tablenum[10]`, column `r df_research_so$table_col[10]`, and row `r df_research_so$table_row[10]`) and `r paste0(df_research_so$param[11], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[11]` - and in the appendix at table `r df_research_so$tablenum[11]`, column `r df_research_so$table_col[11]`, and row `r df_research_so$table_row[11]`) for revealed preference and stated preference, respectively. 

Following the steps described in this section, this approach obtains the same results (`r a4_intgen_rp_pe` and `r paste0(round(irr_intgen_rp_in * 100,2), "%")` for revealed preference and `r a4_intgen_sp_pe` and `r paste0(round(irr_intgen_sp_in * 100,2), "%")` for stated preference, respectively).
...
sim_data1_f <- function(nsims_var2 = 1e2,                   # "Setup" vars
    ...
    gbd_YLD_u5_sim <- data.frame(
      sim = seq(1,nsims_var2),
    ...
    gbd_YLD_5t65_val_sim <- as.data.frame(gbd_YLD_5t65_val_sim)
    gbd_YLD_5t65_val_sim$sim <- seq(1,nsims_var2)
    
    gbd_YLD_5t65_val_sim <- pivot_longer(gbd_YLD_5t65_val_sim, !sim, names_to = "age_c", values_to = "val")
    ...
    gbd_YLD_5t65_pop_sim <- as.data.frame(gbd_YLD_5t65_pop_sim)
    gbd_YLD_5t65_pop_sim$sim <- seq(1,nsims_var2)
    
    gbd_YLD_5t65_pop_sim <- pivot_longer(gbd_YLD_5t65_pop_sim, !sim, names_to = "age_c", values_to = "pop")
    
    # combining the results
    gbd_YLD_5t65_sim <- inner_join(gbd_YLD_5t65_val_sim,gbd_YLD_5t65_pop_sim,by=c("sim","age_c"))
    ...
    colnames(gbd_YLL_5t65_val_sim) <- as.character(gbd_YLL_5t65$age_c)
    gbd_YLL_5t65_val_sim <- as.data.frame(gbd_YLL_5t65_val_sim)
    gbd_YLL_5t65_val_sim$sim <- seq(1,nsims_var2)
    
    gbd_YLL_5t65_val_sim <- pivot_longer(gbd_YLL_5t65_val_sim, !sim, names_to = "age_c", values_to = "val")
    ...
    colnames(gbd_YLL_5t65_pop_sim) <- as.character(gbd_YLL_5t65$age_c)
    gbd_YLL_5t65_pop_sim <- as.data.frame(gbd_YLL_5t65_pop_sim)
    gbd_YLL_5t65_pop_sim$sim <- seq(1,nsims_var2)
    
    gbd_YLL_5t65_pop_sim <- pivot_longer(gbd_YLL_5t65_pop_sim, !sim, names_to = "age_c", values_to = "val")

    # combining the results
    gbd_YLL_5t65_sim <- inner_join(gbd_YLL_5t65_val_sim,gbd_YLL_5t65_pop_sim,by=c("sim","age_c"))
    ...
    # suggested
    # colnames(costs1_counts_sim) <- as.character(costs_data_var2$Country)
    # costs1_counts_sim <- as.data.frame(costs1_counts_sim)
    # costs1_counts_sim$sim <- seq(1,nsims_var2)
    # 
    # costs1_counts_sim <- pivot_longer(costs1_counts_sim, !sim, names_to = "Country", values_to = "total")
...
# suggested
    # colnames(costs1_all_costs_sim) <- as.character(costs_data_var2$Country)
    # costs1_all_costs_sim <- as.data.frame(costs1_all_costs_sim)
    # costs1_all_costs_sim$sim <- seq(1,nsims_var2)
    # 
    # costs1_all_costs_sim <- pivot_longer(costs1_all_costs_sim, !sim, names_to = "Country", values_to = "costs_by_country")
    # costs1_df_sim <- inner_join(costs1_counts_sim,costs1_all_costs_sim,by=c("sim","Country"))


### doesn't look working the below
### 
    ...
    # one_run_f, for the most part, does not include standard deviations   
      invisible( list2env(
        one_run_f(main_run_var1 = FALSE,              
                ...
                # df_costs_var1 = costs1_df_sim[costs1_df_sim$sim==i,], # suggested
                ...
                gbd_YLD_5t65_var1 = gbd_YLD_5t65_sim[gbd_YLD_5t65_sim$sim==i,],
                gbd_YLL_5t65_var1 = gbd_YLL_5t65_sim[gbd_YLL_5t65_sim$sim==i,],
...
| 4.1  | @klps5 only Intergenerational Child Mortality Benefits          | Treatment                      | `r round(a4_intgen_pe, 1)`  | \-                    |`r round(a4_intgen_pe, 1)`  | \-                    |
| **4.2**  | **3.3 + @klps5 (Intergenerational Child Mortality Benefits)**          | **Treatment (EA)**                      | **`r round(a4_inc_a3_pe, 1)`**  | \-                    |**`r round(a4_inc_a3_pe, 1)`**  | \-                    |
...
```{r run-mc, dpi = 400, purl=FALSE, echo=FALSE, eval=FALSE}
npv_all_sim <-   sim_data1_f(
            ...
            ave_death_u5_var2     = ave_death_u5_so,    # app4
            ave_death_u5_sd_var2  = ave_death_u5_so * 0.1,
            gbd_YLD_u5_val_var2   = gbd_YLD_u5$val,
            gbd_YLD_u5_val_sd_var2= gbd_YLD_u5$val * 0.1,
            gbd_YLD_u5_pop_var2   = gbd_YLD_u5$pop,
            gbd_YLD_u5_pop_sd_var2    = gbd_YLD_u5$pop * 0.1,
            gbd_YLD_5t65_val_var2     = gbd_YLD_5t65$val,
            gbd_YLD_5t65_val_sd_var2  = gbd_YLD_5t65$val * 0.1,
            gbd_YLD_5t65_pop_var2     = gbd_YLD_5t65$pop,
            gbd_YLD_5t65_pop_sd_var2  = gbd_YLD_5t65$pop * 0.1,
            gbd_YLL_5t65_val_var2     = gbd_YLL_5t65$val,
            gbd_YLL_5t65_val_sd_var2  = gbd_YLL_5t65$val * 0.1,
            gbd_YLL_5t65_pop_var2     = gbd_YLL_5t65$pop,
            gbd_YLL_5t65_pop_sd_var2  = gbd_YLL_5t65$pop * 0.1,
            cost_per_daly_var2        = cost_per_daly_so,
            cost_per_daly_sd_var2     = cost_per_daly_so * 0.1,
            WTP_child_med_var2        = WTP_child_med_so,
            WTP_child_med_sd_var2     = WTP_child_med_so * 0.1,
            ex_rate_2016_var2         = ex_rate_2016_so,
            ex_rate_2016_sd_var2      = ex_rate_2016_so * 0.1,
            fert_yr_25_var2           = fert_yr_25_in,
            fert_yr_25_sd_var2        = fert_yr_25_in * 0.1,
...
plot1 <- generate_plot_f(npv_all_sim, "A4. All income of A2 and Intergenerational Mortality", rescale_so)[[1]] +
      labs(y = NULL,
       x = "Net Present Value (Benefits - Costs) (USD 2017 PPP)" ,
