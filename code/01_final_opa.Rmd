...
#### Indirect benefits: Including intergenerational effects of child mortality rate reduction

In addition to the benefits of deworming to the treated students, we extrapolate the intergenerational benefits to the children of the dewormed children based on the under-five mortality rate reduction, the disability-adjusted life year (DALY) averted by the mortality rate reduction, and the cost per DALY averted. 

To capture the present value of the benefits of the second generation of each dewormed student, this approach assumes the following points: No pair of treated male and female students become in a relationship. The gains start 15 years after deworming intervention, when the ages of treated students roughly reach the mean age of childbearing, 28.7 years old (United Nations, 2019). To be conservative, this benefit is only gained in the first child of the treated student.
...
```{=tex}
\begin{equation}
fr\sum_{t=15}^{19}{H_t\left(\frac{1}{1+r}\right)^t}
`r equationIndex <- equationIndex + 1`
\label{eq:`r equationIndex`}
\tag{`r equationIndex`}
\end{equation}
...
-   $fr$: fertility rates at age 25-29 (births per 1,000 women)
-   $H_t$: intergenerational benefits to the child of the dewormed student

The DALY averted is calculated by multiplying the reduced mortality rate of under-five children and the years of life lost due to premature mortality (YLL) cited from the Global Burden of Disease (GBD) 2019 study. This report uses $27.42 as the cost per DALY averted in Kenya (PSI, 2020). 

To calculate the intergenerational benefits, we set age categories corresponding to those of the GBD 2019; that is, “under 1 year old” and “1 to 4 years old”. It is assumed that the mortality rate reduction influences only YLL out of DALY.

```{=tex}
\begin{equation}
H_t={CD\times R}_{ac}\times{YLL}_{ac}
`r equationIndex <- equationIndex + 1`
\label{eq:`r equationIndex`}
\tag{`r equationIndex`}
\end{equation}
...
-   $CD$: Cost per DALY averted cited from PSI (2020) (or calculated by 1) health expenditure per capita and Age-standardized rate (ASR) DALY per capita, 2) Willingness-to-pay, or 3) cause-specific cost per DALY and percentage of each cause to the overall DALYs of under 5 children)
-   $R$: intergenerational benefits to the child of the dewormed student
-   $YLL$: the years of life lost due to premature mortality
-   $ac$: age category. This comprises the children under 1 year old and those between age 1 and 4.
