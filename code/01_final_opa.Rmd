...
Without externalities, @baird2016worms obtained a present value of benefits of `r df_research_so$param[1]`, or `r round(currency_f(df_research_so$param[1], t_var=2016), 2)` in 2017 USD PPP (table `r df_research_so$tablenum[1]`, column `r df_research_so$table_col[1]`, and row `r df_research_so$table_row[1]`). Including externalities, they obtain a present value of benefits of `r df_research_so$param[3]`, or `r round(currency_f(df_research_so$param[3], t_var=2016), 2)` (table `r df_research_so$tablenum[3]`, column `r df_research_so$table_col[3]`, and row `r df_research_so$table_row[3]`). Following the steps described in this section, this analysis obtains the same result (`r pv_benef_no_ext_in` and `r pv_benef_yes_ext_in` respectively without rounding).
...
Without externalities, the original analysis (@baird2016worms) obtains a present value of costs of `r df_research_so$param[5] + df_research_so$param[6]`, or `r round(currency_f(df_research_so$param[5] + df_research_so$param[6], t_var=2016), 2)` in 2017 USD PPP (table `r df_research_so$tablenum[5]`, column `r df_research_so$table_col[5]`, and adding rows `r paste0(df_research_so$table_row[5], " and ", df_research_so$table_row[6])`). Including externalities, they obtain a present value of benefits of `r df_research_so$param[5] + df_research_so$param[6] + df_research_so$param[7]` or  `r round(currency_f(df_research_so$param[5] + df_research_so$param[6] + df_research_so$param[7], t_var=2016), 2)` in 2017 USD PPP (table `r df_research_so$tablenum[5]`, column `r df_research_so$table_col[5]`, and adding rows `r paste0(df_research_so$table_row[5], " and ", df_research_so$table_row[6]," and ", df_research_so$table_row[7])`). Following the steps described in this section, this analysis obtains the same result (`r pv_cost_no_ext_in` and `r pv_cost_yes_ext_in`, respectively, without rounding).
...
With complete subsidy, the costs of the intervention become the total direct costs of deworming each child . The second study [@klps4] identifies the unit cost to be \$`r unit_cost_so` per year, in 2018 USD. Adjusting for purchasing power and inflation, the report gets a per capita cost of \$`r round(currency_f(price_var = unit_cost_so, t_var = 2018),2)`. Adding all indirect costs over an average `r round(years_of_treat_0_so,1)` years of treatment, the average cost of deworming each child over the entire treatment period is \$`r format(round(s2_new_in,2), nsmall = 2)`, and after accounting for a take-up rate of `r format(round(q2_in,2), nsmall = 2)` results in an average cost of \$`r format(round(s2_new_in * q2_in,2), nsmall = 2)` (2017 USD PPP).
...
Hence, the cost of schooling each child for an additional year is now \$`r round(currency_f(price_var=teach_sal_new_so, t_var=2017)/n_students_so, 1)` (2017 USD PPP).
...
get_irr <- function(rate_var) {
  ...
  pv_costs_f(
     ...
     s2_var = s2_new_f(
       ...
       unit_cost_local_var = currency_f(price_var = unit_cost_so, t_var = 2018),
       ex_rate_var = 1,
       year_of_treat_var = years_of_treat_t_so
     ),
     q2_var = q_full_so
...
In the KLPS, on average, one person has **`r round(tot_chld_so, 2)` children in a lifetime [@klps5]**. The deworming treatment reduced the under-five mortality rates of children of dewormed students by **`r gamma_mort_so*1000` (per 1,000 children)**. Thus, the treatment to one person roughly averted the death of `r round(tot_chld_so * gamma_mort_so, 2)` children among per treated individual. From a perspective of cost, the direct deworming costs per one student are \$`r round(s2_new_in*q2_in, 2)` (2017 USD PPP), given the treatment period of `r years_of_treat_0_so` years and take-up rate of `r q2_in*100`%. So, we could simply say that `r round(tot_chld_so * gamma_mort_so, 2)` lives can be protected by spending \$`r round(s2_new_in*q2_in, 2)` per treatment to one person. The below sections further explains the costs and benefits of saved children of dewormed students with additional parameters.
...
# Cost per DALY (Mp)
## Revealed Preference - Spring Cleaning (2011 USD) https://academic.oup.com/qje/article-abstract/126/1/145/1903056?redirectedFrom=fulltext
rp_ksh_in <-  currency_f(23.68, t_var = 2011)                                   # EXPLAIN  23.68      
## USD spent in Kenya --(exchange rate)--> KSH: (23.68 * ex_rate_2011_so)
## KSH --(exchange rate)--> USD PPP: rp_ksh_so / ex_rate_2011_ppp_so
## USD PPP --(CPI)--> USD PPP 2017: rp_ppp_so * (cpi_2017_so / cpi_2011_so)

## Stated Preference - Health Status WTP (2016 USD) - Based on DW 21/12/17
sp_in <-      currency_f(162162.2 / ex_rate_2016_so, t_var = 2016)              # EXPLAIN 162162.2

## KSH spent in Kenya ----> KSH: (162162.2/ex_rate_2016_so * ex_rate_2016_so) - WTP_child_med_med *12 (KES) from "Stated_Preference_WTP_Child_Health.Rmd"
## KSH --(exchange rate)--> USD PPP: sp_ksh_so / ex_rate_2016_ppp_so
## USD PPP --(CPI)--> USD PPP 2017: sp_ppp_so * (cpi_2017_so / cpi_2016_so)

#EXPLAIN FOLLOWING SECTION
gbd$age_c <- as.factor(gbd$age_c) 

# sorting
gbd_YLD_u5 <-gbd[gbd$measure == "YLDs (Years Lived with Disability)" & 
                 gbd$age_c %in% c("0-4"), ]
gbd_YLD_u5$age_c <- factor( gbd_YLD_u5$age_c, levels = c("0-4") ) # sorting
gbd_YLD_u5 <- gbd_YLD_u5[order(gbd_YLD_u5$age_c, decreasing = F), ]
# EXPLAIN
...
gbd_YLL_5t65 <-gbd_YLL_5t65[order(gbd_YLL_5t65$age_c, decreasing = F), ]

# EXPLAIN THE DIV BY 5 STEP
...
gbd_YLD_5t65 <-  gbd_YLD_5t65[order(gbd_YLD_5t65$age_c, decreasing = F), ]
...
Adjusted by the inflation and PPP to USD in 2017, the $M_p$ for revealed preference is `r round(currency_f(price_var = cp_daly_rp_so, t_var = 2011), 2)`, and the $M_p$ for stated preference is `r round(currency_f(price_var = cp_daly_sp_so, t_var = 2016), 2)`, respectively (2017 USD PPP). The reader can modify the values for $M_p$ in the interactive app of this OPA. 
  ...
  pv_costs_f(
     ...
     s2_var = s2_new_f(
       ...
       unit_cost_local_var = currency_f(
         price_var = unit_cost_so, t_var = 2018),
       ex_rate_var = 1,
       year_of_treat_var = years_of_treat_t_so),
     q2_var = q_full_so)
...
npv_all_sim <-   sim_data1_f(
            ...
            unit_cost_local_new_var2 = currency_f(price_var = unit_cost_so, t_var = 2018),
            unit_cost_local_new_sd_var2 = currency_f(price_var = unit_cost_so, t_var = 2018) * 0.1  ,  
