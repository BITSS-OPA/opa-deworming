...
###############################################################################  
    ...
    #############
    cp_daly_so <- 27.42 # PSI https://www.psi.org/wp-content/uploads/2020/02/Net-Cost-Per-DALY-2015.pdf
    lambda1_mort_so <- 0.016 # The Treatment Effects. should be aligned with the new paper
    yll_pc_so <- 11870264.06/5993267/0.016/65 # (YLL of age 0-64 in Kenya in 2019)/(Estimated Saved Population of age 0-4 = The Treatment Effects * The Population of age 0-4 in Kenya in 2019)/(65 years) GBD and KNBS http://housingfinanceafrica.org/app/uploads/VOLUME-III-KPHC-2019.pdf
    fert_yr_so <- c(0.003093735, 0.014040798, 0.027129678, 0.043074314, 0.076867422, 0.103521141, 0.140883944, 0.166823724, 0.215847529, 0.249640637, 0.261301639, 0.263205477, 0.268916988, 0.234647920, 0.193001484, 0.161826152, 0.135648392, 0.120179716, 0.081151056, 0.081864995, 0.077819341, 0.067586216, 0.057591072)
    periods_chldb_so <- 22
...
The coverage ($R$) is defined as the fraction, among all neighboring schools (within a 6 km radius) treated within the study. Since the treatment was applied to approximately two-thirds of the population, $R = `r round(coverage_so, 2)`$[^5].
...
With complete subsidy, the costs of the intervention become the total direct costs of deworming each child (in USD). The original study [@baird2016worms] identifies the unit cost to be \$`r unit_cost_so` per year. Adjusting for purchasing power and inflation, the report gets a per capita cost of \$`r round(unit_cost_2017usdppp_so,2)`. Adding all indirect costs over an average `r round(years_of_treat_0_so,1)` years of treatment, the average cost of deworming each child over the entire treatment period is \$`r format(round(s2_new_in,2), nsmall = 2)`, and after accounting for a take-up rate of `r format(round(q2_in,2), nsmall = 2)` results in an average cost of \$`r format(round(s2_new_in * q2_in,2), nsmall = 2)`.

#### Indirect costs: additional years of education and its costs for government

The indirect cost on the education system is calculated similarly to approach 1: the cost per student is multiplied by the increase in school attendance due to deworming. The cost of additional schooling is computed as the the annual cost of schooling per children times the number of additional years of schooling due to deworming. This analysis assumes that pressure is added to educational institutions for a maximum of nine years, starting at year zero, for a total of 10 years. The cost per student ($K$) is updated with new information on annual teacher salary (including benefits)[^7], \$`r format(round(teach_sal_2017usdppp_so,0), scientific=FALSE, big.mark = ",")` (also adjusted for PPP), and the same average number of students per teacher (`r n_students_so`).
...
The second approach does not report benefits and costs separately. With all these elements the main result from the original analysis that is comparable with the results discussed here is the NPV of `r df_research_so$param[9]` (table `r df_research_so$tablenum[9]`, column `r df_research_so$table_col[9]`, and row `r df_research_so$table_row[9]`) This result corresponds to a social internal rate of return of `r paste0(df_research_so$param[8], "%")` (located as an inline result in the paper - also in `r df_research_so$figure_num[8]` - and in the appendix at table `r df_research_so$tablenum[8]`, column `r df_research_so$table_col[8]`, and row `r df_research_so$table_row[8]`). Following the steps described in this section, this analysis obtains the same result (`r a2_all_pe` and `r paste0(irr_in * 100, "%")`, respectively, without rounding).
...
## Approach 4: Combination of Previous Approaches and Intergenerational Benefits of Child Mortality Rate Reduction
...
The benefits are calculated as a monetized value of years of life saved per dewormed individual ($IB$). The calculation is conducted as the discounted sum of fertility rates in t years after the deworming ($fr_{t}$) times the treatment effects on the under 5 mortality reduction of children of the dewormed cohort ($γ_{t}$), the average annual value of saved life per child of the dewormed population ($H$), and the monetary value of health benefits ($M$). This approach assumes that the benefits are added for a maximum of 22 years after the deworming intervention, which is analyzed in **XXX(2021)**.

The fertility rate ($fr_{t}$) signifies the number of children born per dewormed individual t years after the deworming intervention. This number is normalized by the distribution of the childbirth from 1998 to 2020 and the total number of children one dewormed individual bears during the period after the intervention (**XXX(2021). table XXX, column XXX, row XXX**: 1998 as t = 0). The treatment effects on the under 5 mortality reduction of children of the dewormed cohort ($γ_{t}$) are the difference between the mortality rate of under 5 children of the control group and that of the treatment group (**XXX(2021). table XXX, column XXX, row XXX**). The average annual value of saved life per child of the dewormed population ($H$) is the average annual per-capita years of life lost due to premature mortality (YLL) at age 0-64 based on the assumption that the survived child will live up to age 64 if the deworming treatment to their parents prevents their death of under 5 (close to the life expectancy at birth: 66.18 (@united2019world; Life Expectancy at Birth (e0) - Both Sexes, column 2015-2020, row Kenya. Accessed July 12, 2021). We cited YLL of 0-64 years old of all causes and both sexes in Kenya in 2019 from the Global Burden of Disease (GBD) 2019 study (@gbd; Location Kenya, Year 2019, Context Cause, Age  <1 year to 60 to 64, Metric Number, Measure YLLs, Sex Both, Cause Totall All causes, Accessed July 12, 2021) and divided it by the estimated saved under 5 population and the length of the period (65 years) to get the target per-capita value of saved life ($H$). The monetary value of health benefits ($M$) is the saved cost when one YLL is averted. This report uses $27.42 as the cost per YLL averted in Kenya (@psi2015. table Net Cost per DALY Averted, 2015, column Net Cost per DALY Averted, row Kenya, Accessed June 18, 2021). The discounting rate is 5%, as used for approaches 2 and 3. To be conservative, this approach does not consider the factors generated when both parents are dewormed although additional effects might be produced if both male and female parents get treatment. This approach assumes that the benefits are only gained in the child of the treated student through the reduced mortality rates of their children. Estimating the direct health benefits to the treated students is not straightforward. Parasitic worm infections are not lethal, which makes it difficult to quantify the effects of deworming. For further extension of OPA, it is worth adding the direct health benefits.
...
```{=tex}
\begin{equation}
IB = \sum_{t=0}^{22} fr_{t} γ_{t} H \left(  \frac{1}{1 + r}\right)^{t} M\\
`r equationIndex <- equationIndex + 1`
\label{eq:`r equationIndex`}
\tag{`r equationIndex`}
\end{equation}
...
-   $fr_{t}$: the number of children one dewormed individual bears $t$ years after the deworming intervention\
-   $γ_{t}$: the treatment effects on the under 5 mortality reduction of children of the dewormed cohort
-   $H$: the annual average value of life per survived child of the dewormed population. Years of life lost due to premature mortality (YLL) at age 0-64 in Kenya in 2019 divided by the possible saved under 5 population and 65 years
-   $M$: the monetary value of health benefits per $H$, cited from @psi2015 **(or calculated by 1) health expenditure per capita and Age-standardized rate (ASR) DALY per capita, 2) Willingness-to-pay, or 3) cause-specific cost per DALY and percentage of each cause to the overall DALYs of under 5 children)**

For example: if 0.003 children are born per dewormed individual 0 years after the deworming ($fr_{t = 0}$), treating their parents at early ages with deworming reduced their child’s mortality rate by 1.6% ($γ_{t}$), and one survived child in Kenya averts 1.904422988 YLLs annually on average at age 0-64 ($H$), we can estimate the discounted monetized value for one dewormed student at the time of $t = 0$ by multiplying \$27.42 as the cost per DALY averted ($M$). Then, we sum up the benefits using the fertility rate in a specific year $t$ ($fr_{t}$: for $t = 0, 1, ...,22$ ) :

```{=tex}
\begin{equation}
IB = 0.003 \times 0.016 \times 1.904422988 \times \left(  \frac{1}{1 + 0.05}\right)^{t = 0} \times \$27.42 + \dots \\
\dots + 0.261301639 \times 0.016 \times 1.904422988 \times \left(  \frac{1}{1 + 0.05}\right)^{t = 10} \times \$27.42 + \dots \\
\dots + 0.057591072 \times 0.016 \times 1.904422988 \times \left(  \frac{1}{1 + 0.05}\right)^{t = 22} \times \$27.42\\
\end{equation}
```

</details>

```{r intgen_benefits, echo=print_code}
# - inputs: number of childbirth per dewormed individual year by year(fert_yr_so), new interest rate (interest_new_in), number of periods for childbearing of dewormed individual(periods_chldb_so), the treatment effects on under 5 mortality rate reduction (lambda1_mort_so), the years of life lost due to premature mortality per survived child in Kenya(yll_pc_so), Cost per DALY averted(cp_daly_so)
...
###############################################################################  
  intgen_hlth_f <- function(
    ...
    cp_daly_var){
      ...
      res1 <- sum(fert_yr_var * lambda1_mort_var * yll_pc_var * cp_daly_var * (1 / (1 + interest_r_var) )^index_t)
      return(res1)   
    }
...
intgen_hlth_in <- intgen_hlth_f(
                    ...
                    cp_daly_var = cp_daly_so)

```

</details>

Under approach 4, and using the same assumptions as above, the present value of intergenerational benefits is $`r intgen_hlth_in`.
