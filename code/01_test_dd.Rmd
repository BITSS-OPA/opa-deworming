...
The coverage, $R$, is defined as the fraction, among all neighboring of schools (within 6 km), that belongs to the treatment group (last paragraph of page 9(1645) of paper). $R$ is computed from the data fraction of treated primary school students. It is computed from the data as: 

\begin{equation}
R \equiv \frac{\text{Overall Saturation}  }{Q(full)}
\end{equation}

*Overall Saturation* is set at `r saturation_val` [ASK TED ABOUT A DEFINITION FOR THIS PARAM]. And $Q(full)$ is the take-up of the treatment with full subsidy, taken from the literature to be at `r q_full_val` (Miguel and Kremer [add page, table, col, row]).
