...
The estimated externality effect ($\lambda_{2}$) reflects the additional hours worked due to individuals who did not recieve the treatment but still saw reductions in the likelihood of infection due to increased immunity in their community.  Note that this parameter is not estimated by gender, so we repeat its value two times. All the components to the equation \\ref{eq:`r equationIndex + 1`} come from @baird2016worms. The externalities effects are adjusted by the coverage and saturation of the original study. 
...
The coverage ($R$) is defined as the fraction, among all neighboring schools (within 6 km), that belongs to the treatment group. As the treatment was applied to approximately two thirds of the population, $R$ is set to: $R  = `r round(coverage_so, 2)`$.  
...
For this (or similar?) setting Miguel and Kremer 2007 [add page, table, col, row] estimate that there is almost no take-up without subsidy ($Q(0)$), hence  is assigned the value of `r q_zero_so`. The same article [add page, table, col, row] estimates that take-up with full subsidy ($Q(full)$) was of `r q_full_so`.
...
#why are the treatment effects in different columns?
table_1[8,2] <- paste0("$t=" , NA, "$")
table_1[9,2] <- paste0("$\\alpha^{KLPS4}=" , round(lambda1_new_so[3],2), "$")
table_1[1,3] <- paste0("$\\alpha^{KLPS2}=" , round(lambda1_new_so[1],2), "$")

table_1 <- rbind(table_1, c(paste0("$E_t=" , earnings2_f(), "$"), "", ""),
                          c(paste0("$\\alpha^{KLPS3}=" , round(lambda1_new_so[2],2), "$"), "", ""))

sum_table_index <- sum_table_index+1
table_2 <- rbind(table_2,
                 c("$E_t = \\mathbf{1}(10 \\text{<} t \\leq 15)\\alpha^{KLPS2} + \\mathbf{1}(15 \\text{<} t \\leq 20)\alpha^{KLPS3} + \\mathbf{1}(t \\text{>} 20)\\alpha^{KLPS4}$",#fix greater than/ less than symbols
                   paste0("$(", sum_table_index, ")$") )
    )
...
Over this nine year period, students attended school for an additional `r round(sum(delta_ed_so[,1]),2)` years on average. Then we get an average cost of additional schooling per child over the nine-year period, $`r format(round((cost_per_student_in_new*sum( (1/(1+interest_19))^(0:8) * delta_ed_in )),2), nsmall = 2)`.
...
table_1[10,2] <- paste0("$S_1=", NA, "$")
table_1[11,2] <- paste0("$S_2=", NA, "$")
table_1[12, 2] <- paste0("$\\Delta \\overline{E}_{t}(S1,S2)=", NA, "$") #delta_ed_so[,1] is a series
...
In the original evaluation, the prevalence rates where very high (`r alpha_0_so`), hence the effect on the infected population was similar to that of the ovearll population. Currently deworming interventions are discussed in geographies with much lower prevalence rates, hence to obtain the expected effect over the new region, we need to multiply the effect on the infected population by the prevalence rate in the new region ($\eta_{r}$)
