...
Without externalities, the original analysis (@baird2016worms) obtains a total NPV of benefits of `r df_research_so$param[1]`, with `r df_research_so$param[2]` in tax revenue for government (table `r df_research_so$tablenum[1]`, column `r df_research_so$table_col[1]`, and rows `r paste0(df_research_so$table_row[1], ", ", df_research_so$table_row[2])` respectively).
...
Gains in earnings represent the treatment effect on welfare, which implicitly takes into consideration the life cycle profile of wages, economywide growth, etc. @klps4 assumes that the effect on welfare identified 20 years after the intervention persists through one's working life[^8]. This approach also disregards externality effects. Their estimated treatment effects ($\alpha^{KLPS}$) for years 10, 15, and 20, are `r paste0("$", round(mean(lambda1_new_so),0) )` dollars per person per year.
