...
library(rootSolve)
(multiroot(function(x) get_irr(rate_var = x), .2, maxiter=10000000, positive = T))$root
...
Without externalities, the original analysis (@klps4) obtains a total NPV of benefits of `r df_research_so$param[1]`, with `r df_research_so$param[2]` in tax revenue for government (table `r df_research_so$tablenum[1]`, column `r df_research_so$table_col[1]`, and rows `r paste0(df_research_so$table_row[1], ", ", df_research_so$table_row[2])` respectively).
...
Evidence Action is an organization that provides technical assistance to governments to deliver school-based deworming programs. One key role that Deworm the World plays is maintaining close relationships with policymakers who are responsible for ensuring the implementation of deworming programs within their geographies. Deworm the World works to gain and maintain critical support amongst these key stakeholders, thus having important influence over how policymakers take-in and use evidence for decision making. As such, Evidence Action also has access to country-level government cost data on what it takes to implement and evaluate school-based deworming programs across different contexts. To estimate the costs in this analysis, we use Evidence Action's costs of deworming and then follow a similar approach to @givewell, which includes an additional estimate around the amount of government staff time required to run deworming programs. The default cost is the per unit cost per treatment round per child across all countries. This is obtained as the weighted average of per unit costs ($c_{i}$) in all countries were Evidence Action currently has data on implementation of deworming interventions.
...
```{r sum-table21, purl=FALSE, echo=FALSE, eval=TRUE}
...
sum_table_index <- sum_table_index+1
table_2 <- rbind(table_2,
                 c("$CEA_{deworming} = \\frac{B (1 + F_{0})}{C}$",
                   paste0("$(", sum_table_index, ")$") )
    )
sum_table_index <- sum_table_index+1
table_2 <- rbind(table_2,
                 c("$RCEA = \\frac{CEA_{deworming}}{CEA_{cash}}$
",
                   paste0("$(", sum_table_index, ")$") )
    )
...
- **CEA format:** `r round(a3_mpe_cea,1)`    

- **RCEA format (relative to cash):** `r round(a3_mpe_rcea,1)`    
