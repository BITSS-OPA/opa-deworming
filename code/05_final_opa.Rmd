...
For this (or similar?) setting @kremer2007illusion (Page 48, Table 1, Panel C, Col 1, Row 3) estimate that take-up with full subsidy ($Q(full)$) was `r q_full_so`. Miguel and Kremer (2004) (Table 3 and footnote 18) observed minimal to no take-up without subsidy ($Q(0)$), hence it is assigned the value of `r q_zero_so`.  
...
table_1[1,2] <- paste0("$p=" , round(unlist(saturation_in_f(coverage_var = coverage_so, q_full_var = q_full_so, q_zero_var = q_zero_so)),2), "$")
table_1[2,2] <- paste0("$R=" , round(coverage_so,2), "$")
table_1[3,2] <- paste0("$\\lambda_1=" , lambda1_in_f(lambda1_var = lambda1_so)[1], "$")
table_1[4,2] <- paste0("$\\lambda_2=" , lambda2_so, "$")

table_1 <-rbind(table_1,
                c( "", paste0("$\\hat{\\beta}_1=" , round(coef_exp_so[1],2), "$"), ""),
                c( "", paste0("$\\hat{\\beta}_2=" , round(coef_exp_so[2],2), "$"), "") )

table_1[5,1] <- paste0("$w_{ww}=" , round(wage_ww_so,2), "$")
table_1[6,1] <- paste0("$w_{se}=" , round(profits_se_so / (4.5 * hours_se_cond_so),2), "$") #copied from 589 in wage_0_mo but wage_se is never calculated outside the fn

table_1 <-rbind(table_1,
                c( paste0("$h_{ag}=" , hours_ag_so, "$"), "", ""),
                c( paste0("$h_{ww}=" , hours_ww_so, "$"), "", ""),
                c( paste0("$h_{se}=" , hours_se_so, "$"), "", ""),  
                c( paste0("$Q(full)=" , q_full_so, "$"), "", ""),
                c( paste0("$Q(0)=" , q_zero_so, "$"), "", "")
                )
...
Without externalities, the original analysis obtains a total NPV of benefits of `r df_research_so$param[1]`, with `r df_research_so$param[2]` in tax revenue for government (table `r df_research_so$tablenum[1]`, column `r df_research_so$table_col[1]`, and rows `r paste0(df_research_so$table_row[1], ", ", df_research_so$table_row[2])` respectively).

Including externalities, they obtain a total NPV of benefits of `r df_research_so$param[3]`, with `r df_research_so$param[4]` in tax revenue for government (table `r df_research_so$tablenum[3]`, column `r df_research_so$table_col[3]`, and rows `r paste0(df_research_so$table_row[3], ", ", df_research_so$table_row[4])` respectively).
...
Gains in earnings represent the treatment effect on welfare, which implicitly takes into consideration the life cycle profile of wages, economywide growth, etc. @klps4 assumes that the effect on welfare identified 20 years after the intervention persists through one's working life[^8]. This approach also disregards externality effects. Their estimated treatment effects ($\alpha_{t}$) for years 10, 15, and 20, are `r paste0("$", round(lambda1_new_so,0) )` dollars per person per year.
...
table_1[8,2] <- paste0("$\\alpha^{KLPS2}=" , round(lambda1_new_so[1],2), "$")
table_1[9,2] <- paste0("$\\alpha^{KLPS3}=" , round(lambda1_new_so[2],2), "$")
table_1[10,2] <- paste0("$\\alpha^{KLPS4}=" , round(lambda1_new_so[3],2), "$")

sum_table_index <- sum_table_index+1
table_2 <- rbind(table_2,
                 c("$\\Delta W_{t} = \\mathbf{1}(10 \\lt t \\leq 15)\\alpha^{KLPS2} + \\mathbf{1}(15 \\lt t \\leq 20)\\alpha^{KLPS3} + \\mathbf{1}(t \\gt 20)\\alpha^{KLPS4}$",
                   paste0("$(", sum_table_index, ")$") )
    )
...
table_1[11,2] <- paste0("$r_{16}=", interest_16, "$")
...
table_1[12,1] <- paste0("$\\overline{{C}_{i}}=", round(costs1_p2_f(country_total_var = costs_data$total,
                         country_cost_var = costs_data$costs_by_country,
                         staff_time_var = staff_time_so), 2), "$") #weighted average
table_1[13,1] <- paste0("$\\overline{{N}_{i}}=", round(mean(costs1_p1_in$total)), "$")
