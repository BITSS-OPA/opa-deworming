...
This report is part of an Open Policy Analysis (OPA) on deworming interventions. An OPA contains one open output that best represents the facts to inform policy makers, on report (this one) that clearly explains all the analysis, and one repository that contains all the materials to reproduce the report and final output. 
...
This report is part of an Open Policy Analysis (OPA) project on deworming interventions. A framework for making policy analyses transparent and reproducible (Hoces de la Guardia et al., 2020), OPA’s goal is to clearly show how an analysis was conducted and how to best represent key figures or results for policy makers to use as a factual basis for deliberation. In addition, OPA facilitates the re-use of analyses across similar settings, and sheds light on how evidence generated by research is used in specific policy analyse
...
The actual value varies across approaches depending on the time and country chosen. For example approach 1 used the return from government bonds and the inflation rate in Kenya for the year 2016, while approach 3 used the values for the same country for the year 2019. This results in discount rates of `r paste(round(100 * interest_16,2), "%", sep="")` and `r paste(round(100 * interest_19,2), "%", sep="")` for approach 1 and 3 respectively.
...
The coverage ($R$) is defined as the fraction, among all neighboring schools (within 6 km), that were treated within the study. Since the treatment was applied to approximately two thirds of the population, $R$ is set to: $R  = `r round(coverage_so, 2)`$.  
...
For this setting @kremer2007illusion (Page 48, Table 1, Panel C, Col 1, Row 3) estimate that take-up with full subsidy ($Q(full)$) was `r q_full_so`. Miguel and Kremer (2004) (Table 3 and footnote 18) observed minimal to no take-up without subsidy ($Q(0)$), hence it is assigned the value of `r q_zero_so`.  
...
```{r sum-tables3, purl=FALSE, echo=FALSE, eval=TRUE, , eval=show_summary_tables}
...
```{r sum-tables16, purl=FALSE, echo=FALSE, eval=TRUE, eval=show_summary_tables}
...
Gains in yearly earnings represent the treatment effect on welfare ($\alpha$), which implicitly takes into consideration the life cycle profile of wages, economywide growth, etc. @klps4 assumes that the effect on welfare identified 20 years after the intervention persists through one's working life[^8]. This approach also disregards externality effects. The estimated treatment effects that pools years 10, 15, and 20, is `r paste0("$", round(mean(lambda1_new_so),0) )` dollars per person per year.
...
```{r sum-tables17, purl=FALSE, echo=FALSE, eval=TRUE, eval=show_summary_tables}
...
Over this nine year period, treated students attended school for an additional `r round(sum(delta_ed_so[,1]),2)` years on average. Then we get an average cost of additional schooling per child over the nine-year period, $`r format(round((cost_per_student_in_new*sum( (1/(1+interest_19))^(0:8) * delta_ed_in )),2), nsmall = 2)`.
...
table_1[9,2] <- paste0("$r_{16}=", interest_16, "$")

table_1 <- rbind(table_1, c("", paste0("$r_{19}=", interest_19, "$"), ""))
...
In the original deworming study conducted in Kenya in 1999, the prevalence rates of worm infections were up to `r paste0(round(prevalence_0_so * 100), "%")` for the relevant population and costs were estimated at $0.49 per treatment. As of 2020, Evidence Action supports deworming interventions in four countries.
...
Under apporach 3, and using the same assumptions as above, the benefits will be the same as in approaches 1 and 2: `r round(app3_pv_benef_no_ext_in, 2)` and `r round(app3_pv_benef_yes_ext_in, 2)` for approach 1 without and with externalities, and `r round(app3_pv_benef_all_new_in, 2)` when using the benefit structure of approach 2.  
...
The unit costs of treatments, although small, vary substantially across regions. When including cost information for all the countries where Evidence action has data (India, Kenya, Nigeria, Vietnam) the unit costs is `r paste0("$", round(costs1_p2_in, 2))` per round of treatment. This final costs is primarily driven by the costs, and large population, of India, with a unit cost of `r paste0("$", round(costs1_p2_f(select_var = list("india")), 2 ))`, the other 3 remaining countries have relatively larger unit costs: `r paste0("$", round(costs1_p2_f(select_var = list("kenya")), 2 ))`, `r paste0("$", round(costs1_p2_f(select_var = list("nigeria")), 2 ))`, `r paste0("$", round(costs1_p2_f(select_var = list("vietnam")), 2 ))` for Kenya, Nigeria and Vietnam respectively. 
...
one_run <-
  function(main_run_var1 = main_run_so,
    ...
    # # ###example of condensing earnings1 computation using mapply
    # # #putting arguments into vector/list
    # earnings_lambda1_var = list(lambda1_in[1], lambda1_in[1], lambda1_prev_in[1], lambda1_prev_in[1])
    # earnings_lambda2_var = list(0, lambda2_in[1],0,lambda2_in[1])
    # 
    # # #computing earnings1 (above code) using mapply. Result is a matrix where column 1 is earnings_in_no_ext and so forth.
    # earnings_1f_lapply_result = mapply(earnings1_f,earnings_lambda1_var, earnings_lambda2_var, MoreArgs = list(wage_var = wage_t_in,
    #                                    saturation_var = saturation_in, coverage_var = coverage_var1))
    # #checking if mapply worked
    # all(earnings_1f_lapply_result[,1] == earnings_in_no_ext)
    # all(earnings_1f_lapply_result[,2] == earnings_in_yes_ext)
    # all(earnings_1f_lapply_result[,3] == earnings_in_no_ext_prev)
    # all(earnings_1f_lapply_result[,4] == earnings_in_yes_ext_prev)
    ...
    # ###example of condensing earnings2 computation using lapply
    # #putting arguments into vector/list
    # earnings_2f_test = list(lambda1_new_in[1], lambda1_prev_new_in[1])
    # #
    # #computing earnings2_f using lapply
    # earnings2f_lapply_result = lapply(earnings_2f_test,earnings2_f, t_var = 0:50)
    ...
    ###example of condensing unit_test using mapply.
    # #putting arguments into equal length vectors/lists
    # earnings_inputs_test = list(earnings_in_no_ext, earnings_in_yes_ext, earnings_in_no_ext_prev, earnings_in_yes_ext_prev)
    # earnings_inputs_test2 = list(31.1421332040266, 167.667817450905, 16.9694876943406, 153.495171941219)
    # 
    # #computing unit_test using arguments in lists above
    # mapply(unit_test, earnings_inputs_test, earnings_inputs_test2, main_run_var = main_run_var1)
...
- NPV without externalities in @baird2016worms ($\lambda_2 = 0$): `r round(a1_all,1)`    

- NPV with externalities in @baird2016worms ($\lambda_2 = `r round(lambda2_so,2)`$ ): `r round(a1_x_all,1)`  
...
rescale <- rescale_so
    ...
    plot1 <- ggplot() +
      geom_density(aes(x = npv_sim,
                       alpha = 1/2, ..scaled..), lwd = 0.8, kernel = "gau") +
      geom_vline(xintercept = c(0, median(npv_sim)), col=c("black", "blue"), lwd = c(0.8, 0.8) ) +
