...
**Then we get an average cost of additional schooling per child over the nine-year period, $`r format(round((cost_per_student_in*sum( (1/(1+0.05))^(0:8) * delta_ed_in )),2), nsmall = 2)`.**
...
```{r final-output, echo=print_code, purl=FALSE}
...
- **NPV without externalities in Baird et al, 2016 ($\lambda_2 = 0$):** `r round(baird3,1)`    

- **NPV with externalities in Baird et al, 2016 ($\lambda_2 = `r round(lambda2_so,2)`$ ):** `r round(baird4,1)`
...
- **NPV without externalities in KLPS 2019:** `r round(klps4_2,1)`    

- **CEA without externalities in EA:** `r round(cea_no_ext_ea,1)`    

- **RCEA without externalities in EA (relative to cash):** `r round(rcea_no_ext_ea,1)`    
