...
\underbrace{\left[\sum_{t=0}^{`r round(years_of_treat_so,0)`} \left( \frac{1}{1 + r}\right)^{t} \big[S_{2}Q(S_{2}) - S_{1}Q(S_{1}) \big]\right]}_{\text{cost of deworming medication}}
\label{eq:1}
\tag{1}
\end{equation}
...
\underbrace{\left[\sum_{t=0}^{`r round(years_of_treat_so,0)`} \left( \frac{1}{1 + r}\right)^{t}w_t - \sum_{t=0}^{`r round(years_of_treat_so,0)`} \left( \frac{1}{1 + r}\right)^{t}\big[S_{2}Q(S_{2}) - S_{1}Q(S_{1}) \big]\right]}_{\text{short-term NPV (direct health effects)}}
\end{equation}
