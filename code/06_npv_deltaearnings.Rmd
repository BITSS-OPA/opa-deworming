...
\begin{equation}
NPV =
-\underbrace{\left[\sum_{t=0}^{`r round(years_of_treat_so,1)-1`} \left( \frac{1}{1 + r}\right)^{t} \big[S_{t,2}Q(S_{t,2}) - S_{t,1}Q(S_{t,1}) \big]\right]}_{\text{cost of deworming medication}}
- \underbrace{\left[K \sum_{t=0}^{`r dim(delta_ed_so)[1]-1`} \left( \frac{1}{1 + r}\right)^{t} \Delta \overline{E}_t(S1,S2)\right]}_{\text{cost of schooling}}
...
Each term in the model is discounted by $r$, which is set to the real interest rate [**rationale?**], obtained from the interest rate on goverment bonds (`r round(gov_bonds_so, 3)`) minus the inflation rate (`r inflation_so`).
...
The resulting value is $r$ = `r paste(round(100*interest_in,2), "%", sep="")`.

## Cost of deworming medication

The costs of deworming medication is obtained by the sum of discounted costs of deworming over the treatment period. The average treatment period in this study was `r years_of_treat_so` years, which we round to `r round(years_of_treat_so,1)`. So starting year zero, we have
...
**So we get an average cost of deworming each child over the entire treatment period, $`r format(round(sum( ( 1 / (1 + interest_in) )^(0:2) * (s2_in * q2_in) ),2), nsmall = 2)`.**
...
$K$ represents the cost of schooling each child for an additional year ($`r round(teach_sal_2017usdppp_so/n_students_so, 2)`). It is calculated by dividing an estimate of annual teacher salary by the number of average number of students per teacher.

\begin{equation}
K = \frac{\text{teacher salary}}{\text{number students}}
\end{equation}

Annual teacher salary ($`r format(round(teach_sal_2017usdppp_so,0), scientific=FALSE)`) is based on the upper tier of monthly teacher salaries reported by two Kenyan news sources: Nyanchama (2018) and Oduor. Since compensation for teachers in rural villages where the treatment was administered is below the national average, we are overestimating the costs for a conservative analysis. The average number of students per teacher is `r n_students_so`, based on **[FILL IN]**.
...
**Then we get an average cost of additional schooling per child over the nine-year period, $`r format(round((cost_per_student_in*sum( (1/(1+interest_in))^(0:8) * delta_ed_in )),2), nsmall = 2)`.**
...
\begin{equation}
NPV =
-\underbrace{\left[\sum_{t=0}^{`r round(years_of_treat_so,0)-1`} \left( \frac{1}{1 + r}\right)^{t} \big[S_{2}Q(S_{2}) - S_{1}Q(S_{1}) \big]\right]}_{\text{cost of deworming medication}}
+ \underbrace{\left[\sum_{t=0}^{`r round(years_of_treat_so,0)-1`} \left( \frac{1}{1 + r}\right)^{t}w_t \right]}_{\text{direct health effects}}
