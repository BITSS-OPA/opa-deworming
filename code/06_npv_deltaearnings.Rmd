...
Teacher salary is estimated by the average salary of teachers in Kenya ($`r round(teach_sal_so*(cpi_2017_so/cpi_2007_so),0)`). Likewise, teacher benefits are estimated by the average benefits of teachers in Kenya ($`r round(teach_ben_so*(cpi_2017_so/cpi_2007_so),0)`). Each of these have been adjusted for inflation. Since compensation for teachers in rural villages where the treatment was administered is below the national average, we are overestimating the costs for a conservative analysis. The average number of students per school is (`r n_students_so`).
...
The earnings gains observed 10, 15, and 20 years from the intervention are estimated to be `r lambda1_so` dollars per person per year respectively. We assume there are no earnings gains in the first `r time_to_jm_so` years after receiving deworming medication, and earnings gains persist through the end of one's working life (`r periods_so` years after receiving treatment) or die out after the last observed five-year period (25 years after receiving treatment). The annual tax on earnings is assumed to be `r 100*round(tax_so,3)`%.

The cost of additional schooling is given by the product of the cost of schooling each child and the additional years of schooling. The cost of schooling each child for an additional year ($`r round(cost_per_student_in,2)`) is calculated by dividing the sum of annual teacher salary ($`r teach_sal_so`) and teacher benefits ($`r teach_ben_so`) by the number of average number of students per teacher (`r n_students_so`). And on average, treated individuals had `r round(mean(delta_ed_so[,1]),2)` additional years of schooling.
...
The cost of deworming one person for one year is KSH`r unit_cost_so` and the treatment was administered for an average of `r years_of_treat_so` years.

All three values are given in PPP units (where the PPP exchange rate from KSH to USD is `r round(ex_rate_ppp_so,2)`), and the costs and benefits of schooling are adjusted for inflation. The cost of deworming medication is not adjusted for inflation since the cost is negligible and has, in fact, decreased over time. [SOURCE?]
...
\* The average observed earnings gains is `r round(mean(lambda1_so),2)`.
