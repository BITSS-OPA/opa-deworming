...
The treatment effect on earnings observed 10, 15, and 20 years from the intervention (in each round of data collection) were, on average, `r lambda1_2017usdppp_so` dollars per person per year respectively. We assume there are no earnings gains in the first `r time_to_jm_so` years after receiving deworming medication, and earnings gains persist through the end of one's working life (`r periods_so` years after receiving treatment) or die out after the last observed five-year period (25 years after receiving treatment). The annual tax on earnings is assumed to be `r 100*round(tax_so,3)`% based on **[FILL IN]**.
...
The average cost of deworming per person ($`r round(unit_cost_2017usdppp_so*years_of_treat_so,2)`.00) is given by the product of the annual cost of deworming per person ($`r round(unit_cost_2017usdppp_so,2)`) and number of years over which the treatment was administered on average (`r years_of_treat_so`). The annual cost of deworming per child in Kenya was obtained from a 2018 estimate by Evidence Action.
