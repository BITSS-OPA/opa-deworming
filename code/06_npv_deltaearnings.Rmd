...
The earnings gains observed 10, 15, and 20 years from the intervention are estimated to be `r lambda1_so` dollars per person per year respectively. We assume there are no earnings gains in the first 10 years after receiving deworming medication, and earnings gains persist through the end of one's working life (50 years after receiving treatment) or die out after the last observed five-year period (25 years after receiving treatment). The tax on earnings is assumed to be `r 100*round(tax_so,3)`%.

The cost of additional schooling is given by the product of the cost of schooling each child and the additional years of schooling. The cost of schooling each child for an additional year ($`r round(cost_per_student_in,2)`) is calculated by dividing the sum of teacher salary (`r teach_sal_so`) and teacher benefits (`r teach_ben_so`) by the number of average number of students per teacher (`r n_students_so`).
