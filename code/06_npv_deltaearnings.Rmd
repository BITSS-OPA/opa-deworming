...
Teacher salary is estimated by the average salary of teachers in Kenya ($`r round(teach_sal_2017usdppp_so,0)`). Likewise, teacher benefits are estimated by the average benefits of teachers in Kenya ($`r round(teach_ben_2017usdppp_so,0)`). Each of these have been adjusted for inflation. Since compensation for teachers in rural villages where the treatment was administered is below the national average, we are overestimating the costs for a conservative analysis. The average number of students per school is (`r n_students_so`).
...
|`r 100*interest_in`%             |25 years           |**`r round(npv_int05_die,0)`**             |**`r round(tax_int05_die,0)`**                  |                                        |*                                         |**Notes.** The Net Present Value takes three factors into account: (tax on) earnings gains (a result of additional schooling), the cost of additional schooling, and the cost of deworming medication.

The earnings gains observed 10, 15, and 20 years from the intervention are estimated to be `r lambda1_2017usdppp_so` dollars per person per year respectively. We assume there are no earnings gains in the first `r time_to_jm_so` years after receiving deworming medication, and earnings gains persist through the end of one's working life (`r periods_so` years after receiving treatment) or die out after the last observed five-year period (25 years after receiving treatment). The annual tax on earnings is assumed to be `r 100*round(tax_so,3)`%.

The cost of additional schooling is given by the product of the cost of schooling each child and the additional years of schooling. The cost of schooling each child for an additional year ($`r round(cost_per_student_in, 2)`) is calculated by dividing the sum of annual teacher salary ($`r round(teach_sal_2017usdppp_so,2)`) and teacher benefits ($`r round(teach_ben_2017usdppp_so,0)`) by the number of average number of students per teacher (`r n_students_so`). And on average, treated individuals had `r round(mean(delta_ed_so[,1]),2)` additional years of schooling.
...
The cost of deworming one person for one year is $`r round(unit_cost_2017usdppp_so,2)` and the treatment was administered for an average of `r years_of_treat_so` years.
...
\* The average observed earnings gains is `r round(mean(lambda1_2017usdppp_so),2)`.
